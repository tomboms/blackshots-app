// --- BASKETBAL_ZAALHUUR.JS: STRICTE STATUS & PERIODE FILTERS ---
...
// ============================================================================
// UREN BEREKENEN (KOGELVRIJ TEGEN NaN)
// ============================================================================
function berekenVerschilInUren(start, eind) {
    try {
        // Haalt alleen puur de cijfers (19:00) eruit, negeert woorden zoals "uur"
...
// ============================================================================
// EXCEL BEREKENING (STRICTE KOLOM G CHECK)
// ============================================================================
window.verwerkZaalhuurBestand = function(e) {
    ...
    reader.onload = function(e) {
        ...
        ruweData.forEach((row, idx) => {
            if (!row || row.length < 5 || idx === 0) return; // Sla lege rijen en header over

            // We pakken de kolommen nu EXACT zoals ze in het Sjabloon staan
            ...
            let statusRaw = row[6] ? row[6].toString().trim() : "Geboekt"; // DIT IS KOLOM G!
            ...
            // STATUS BEPALEN (Kijkt ALLEEN naar Kolom G!)
            let status = "Geboekt";
            ...
            // Uren berekenen via de veilige functie
            ...
            window.zaalhuurData.push({
                datum: datum,
                isoDatum: converteerNaarISODatum(datum), // Onzichtbare datum voor de filter!
                startTijd: startTijd, 
                eindTijd: eindTijd, 
                zaaldeel: zaaldeel, 
                zaal: zaal, 
...
// ============================================================================
// INTERACTIEVE UI TEKENEN MET PERIODE FILTERS
// ============================================================================
window.tekenZaalhuurResultaten = function() {
    ...
    Object.keys(groepenPerDag).forEach(dagKey => {
        groepenPerDag[dagKey].sort((a, b) => a.startTijd.localeCompare(b.startTijd));
    });

    // Bouw de HTML weergave
    let html = '';
    Object.keys(groepenPerDag).forEach(dagKey => {
        let items = groepenPerDag[dagKey];
        
        let dagTotaalKosten = items.reduce((som, z) => som + (z.geannuleerd ? 0 : z.bedrag), 0);
        let dagTotaalUren = items.reduce((som, z) => som + (Number(z.uren) || 0), 0);
